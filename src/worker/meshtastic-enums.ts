...
// config.proto (DeviceConfig.Role). Carte/rôle inconnu (réf trop récente pour
// ce catalogue) → numéro brut renvoyé tel quel (comme l'explorer Gaulix).
...
const DEVICE_ROLES: Record<number, string> = {
  0: "CLIENT",
  1: "CLIENT_MUTE",
  2: "ROUTER",
  3: "ROUTER_CLIENT",
...
// Libelle une valeur d'enum. Non numérique → null (champ absent). Numéro hors
// catalogue → la valeur brute en chaîne (on n'invente pas, on garde l'info).
function enumName(map: Record<number, string>, v: unknown): string | null {
  if (typeof v !== "number" || !Number.isFinite(v)) return null;
  return map[v] ?? String(v);
}

export function hardwareModelName(v: unknown): string | null {
  return enumName(HARDWARE_MODELS, v);
}

export function deviceRoleName(v: unknown): string | null {
  return enumName(DEVICE_ROLES, v);
}
